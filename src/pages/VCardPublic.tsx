                  ...
                  {section.section_type === 'testimonial' && section.content?.testimonials && (
                    <div className="space-y-4">
                      {(section.content.testimonials as { name: string; role?: string; company?: string; content: string; rating?: number; avatar?: string }[]).map((testimonial, idx) => (
                        <div 
                          key={idx} 
                          className="bg-gray-50 rounded-2xl p-5 border border-gray-100 relative"
                        >
                          {/* Quote Icon */}
                          <div className={`absolute top-4 right-4 w-8 h-8 rounded-full ${style.accent} flex items-center justify-center opacity-20`}>
                            ...
                            <div className="flex items-center gap-0.5 mb-3">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <svg
                                  ...
                                  viewBox="0 0 24 24"
                                  fill={star <= testimonial.rating! ? '#facc15' : '#e5e7eb'}
                                  className="w-4 h-4"
                                >
